...
pub fn user_decoder() -> decode.Decoder(User) {
  ...
  use starred_repositories <- decode.field("starredRepositories", starred_repository_connection_decoder())
...
pub fn starred_repository_connection_decoder() -> decode.Decoder(StarredRepositoryConnection) {
  ...
  use edges <- decode.field("edges", decode.optional(decode.list(starred_repository_edge_decoder())))
...
pub fn starred_repository_edge_decoder() -> decode.Decoder(StarredRepositoryEdge) {
...
pub fn repository_decoder() -> decode.Decoder(Repository) {
  ...
  use homepage_url <- decode.field("homepageUrl", decode.optional(decode.string))
  ...
  use languages <- decode.field("languages", decode.optional(language_connection_decoder()))
  use latest_release <- decode.field("latestRelease", decode.optional(release_decoder()))
  use license_info <- decode.field("licenseInfo", decode.optional(license_decoder()))
  ...
  use repository_topics <- decode.field("repositoryTopics", repository_topic_connection_decoder())
...
pub fn language_connection_decoder() -> decode.Decoder(LanguageConnection) {
  ...
  use edges <- decode.field("edges", decode.optional(decode.list(language_edge_decoder())))
...
pub fn release_decoder() -> decode.Decoder(Release) {
  ...
  use published_at <- decode.field("publishedAt", decode.optional(decode.string))
...
pub fn repository_topic_connection_decoder() -> decode.Decoder(RepositoryTopicConnection) {
  use total_count <- decode.field("totalCount", decode.int)
  use nodes <- decode.field("nodes", decode.optional(decode.list(repository_topic_decoder())))
...
pub fn user_to_json(input: User) -> json.Json {
  json.object(
    [
      ...
      #("starredRepositories", starred_repository_connection_to_json(
        input.starred_repositories,
      )),
    ],
  )
}

pub fn starred_repository_connection_to_json(input: StarredRepositoryConnection) -> json.Json {
  json.object(
    [
      ...
      #("edges", json.nullable(
        input.edges,
        fn(list) { json.array(from: list, of: starred_repository_edge_to_json) },
      )),
    ],
  )
}
...
pub fn starred_repository_edge_to_json(input: StarredRepositoryEdge) -> json.Json {
  json.object(
    [
...
pub fn repository_to_json(input: Repository) -> json.Json {
  json.object(
    [
      ...
      #("repositoryTopics", repository_topic_connection_to_json(
        input.repository_topics,
      )),
...
pub fn language_connection_to_json(input: LanguageConnection) -> json.Json {
  json.object(
    [
      ...
      #("edges", json.nullable(
        input.edges,
        fn(list) { json.array(from: list, of: language_edge_to_json) },
      )),
    ],
  )
}
...
pub fn repository_topic_connection_to_json(input: RepositoryTopicConnection) -> json.Json {
  json.object(
    [
      #("totalCount", json.int(input.total_count)),
      #("nodes", json.nullable(
        input.nodes,
        fn(list) { json.array(from: list, of: repository_topic_to_json) },
      )),
    ],
  )
}
...
pub type StarredReposResponse {
  StarredReposResponse(viewer: User)
}

pub fn starred_repos_response_decoder() -> decode.Decoder(StarredReposResponse) {
  use viewer <- decode.field("viewer", user_decoder())
  decode.success(StarredReposResponse(viewer: viewer))
}

pub fn starred_repos_response_to_json(input: StarredReposResponse) -> json.Json {
  json.object([#("viewer", user_to_json(input.viewer))])
}

pub fn starred_repos(client: squall.Client, cursor: String) -> Result(Request(String), String) {
  squall.prepare_request(
    ...
    json.object([#("cursor", json.string(cursor))]),
  )
}

pub fn parse_starred_repos_response(body: String) -> Result(StarredReposResponse, String) {
  squall.parse_response(body, starred_repos_response_decoder())
}
